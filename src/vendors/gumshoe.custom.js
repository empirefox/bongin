(function (root, factory) {
	...
	var settings, eventTimeout, docHeight, header, headerHeight, currentNav, sider, siderOffset;
	...
	gumshoe.setDistances = function () {
		...
		if (sider) {
			siderOffset = getOffsetTop(sider);
		}
	...
	var getNavs = function () {
		...
		var navLinks = settings.links || document.querySelectorAll( settings.selector );

		// For each link, create an object of attributes and push to an array
		forEach( navLinks, function (nav) {
			if ( !nav.name ) return;
			var target = document.querySelector( nav.name );
			if ( !target ) return;
			navs.push({
				nav: nav,
				target: target,
				parent: nav.parentNode.tagName.toLowerCase() === 'li' ? nav.parentNode : null,
				out: nav.parentNode.parentNode.parentNode.tagName.toLowerCase() === 'li' ? nav.parentNode.parentNode.parentNode : null,
				distance: 0
			});
		});
	...
	var deactivateCurrentNav = function () {
		if ( currentNav ) {
			...
			if ( currentNav.out ) {
				currentNav.out.classList.remove( settings.activeClass );
			}
		}
	};
	...
	var activateNav = function ( nav ) {
		...
		if ( nav.out ) {
			nav.out.classList.add( settings.activeClass );
		}
	...
	gumshoe.init = function ( options ) {
		...
		sider = document.querySelector( settings.sider );
